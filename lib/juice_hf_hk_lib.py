...
def status_read(cdf, data):
    ...
    if data.sid in [3, 4, 20, 5, 21, 6, 9, 22, 7, 8]:
    ...
    if data.sid in [3]:
        ...
        data.sweep_table    = cdf['sweep_table'][...]
        data.Ver_tbl_freq   = cdf['Ver_tbl_freq'][...]
        data.Ver_tbl_mask   = cdf['Ver_tbl_mask'][...]
    if data.sid in [3, 4, 20, 21]:
        data.complex     = cdf['complex'][...]
    if data.sid in [7, 8, 23]:
        data.freq_center = cdf['frequency'][...]

    if data.sid in [2, 8, 23]:
        data.N_samp     = np.int64(cdf['N_samp'][...])
    if data.sid in [2, 3, 4, 20, 5, 21, 6, 9, 22]:
        data.N_step     = np.int64(cdf['N_step'][...])         # SID-2,3,4/20,5/21,6/22
    if data.sid in [6, 9, 22, 7]:
        data.N_lag      = np.int64(cdf['N_lag'][...])
    if data.sid in [9]:
        data.freq_ID    = np.int64(cdf['freq_ID'][...])
    if data.sid in [20, 7, 8, 23]:
        data.N_block    = np.int64(cdf['N_block'][...])
    if data.sid in [8]:
        data.N_block_ID = np.int64(cdf['N_block_ID'][...])
...
def status_add(data, data1):
    ...
    # HF-ID 
    data.HF_ID            = np.r_["0", data.HF_ID, data1.HF_ID]
    # data.sid              = np.r_["0", data.sid, data1.sid]
    # data.RPWI_FSW_version = np.r_["0", data.RPWI_FSW_version, data1.RPWI_FSW_version]
    # data.HF_config        = np.r_["0", data.HF_config, data1.HF_config]
    ...
    if data.sid in [9]:
        data.freq_ID     = np.r_["0", data.freq_ID, data1.freq_ID]
    if data.sid in [20, 7, 8, 23]:
        data.N_block     = np.r_["0", data.N_block, data1.N_block]
    if data.sid in [8]:
        data.N_block_ID  = np.r_["0", data.N_block_ID, data1.N_block_ID]
    return data
...
def status_shaping(data, index):
    ...
    # HF-ID 
    data.HF_ID            = data.HF_ID[index]
    # data.sid              = data.sid[index]
    # data.RPWI_FSW_version = data.RPWI_FSW_version[index]
    # data.HF_config        = data.HF_config[index]
    ...
    if data.sid in [3]:
        ...
        data.proc_param0    = data.proc_param0[index]
        data.proc_param1    = data.proc_param1[index]
        data.proc_param2    = data.proc_param2[index]
        data.proc_param3    = data.proc_param3[index]
        data.sweep_table    = data.sweep_table[index]
        data.Ver_tbl_freq   = data.Ver_tbl_freq[index]
        data.Ver_tbl_mask   = data.Ver_tbl_mask[index]
    if data.sid in [3, 4, 20, 21]:
    ...
    if data.sid in [2, 3, 4, 20, 5, 21, 6, 22]:
        data.N_step      = data.N_step[index]
    if data.sid in [6, 9, 22, 7]:
        data.N_lag       = data.N_lag[index]
    if data.sid in [9]:
        data.freq_ID     = data.freq_ID[index]
    if data.sid in [20, 7, 8, 23]:
        data.N_block     = data.N_block[index]
    if data.sid in [8]:
        data.N_block_ID  = data.N_block_ID[index]
    return data
