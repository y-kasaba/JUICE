"""
    JUICE RPWI HF Status -- 2025/10/21
"""
import numpy as np
import math
import juice_cdf_lib   as hk_cdf

class struct:
    pass

# ---------------------------------------------------------------------
# --- AUX and Header --------------------------------------------------
# ---------------------------------------------------------------------
def status_read(cdf, data):
    # HF-ID
    data.HF_ID            = cdf['HF_ID'][...]                   # all
    data.sid              = (data.HF_ID[0] >> 8) & 0xFF
    data.RPWI_FSW_version = (data.HF_ID[0] >> 4) & 0xF
    data.HF_config        =  data.HF_ID[0]       & 0xF

    # Common
    data.epoch       = cdf['Epoch'][...]                        # all
    data.scet        = cdf['SCET'][...]                         # all

    # AUX ###
    data.T_RWI_CH1   = np.float32(cdf['T_RWI_CH1'][...])        # all: SID-2,3,4/20,5/21,6/22,7/23
    data.T_RWI_CH2   = np.float32(cdf['T_RWI_CH2'][...])        # all: SID-2,3,4/20,5/21,6/22,7/23
    data.T_HF_FPGA   = np.float32(cdf['T_HF_FPGA'][...])        # all: SID-2,3,4/20,5/21,6/22,7/23
    data.cal_signal  = cdf['cal_signal'][...]                   # all: SID-2,3,4/20,5/21,6/22,7/23
    #       cdf['Unique_ID']
    #       cdf['header_size']
    if data.sid in [2, 3, 4, 20, 5, 21, 6, 22, 7]:
        data.ch_selected   = cdf['ch_selected'][...]            # SID-2,3,4/20,5/21,6/22,7
    # if sid in [2, 3, 4, 20, 5, 21]:
    #   cdf['sweep_table']
    #   cdf['FFT_window']
    #   cdf['onboard_cal']
    #   cdf['proc_param0']
    #   cdf['proc_param1']
    #   cdf['proc_param2']
    #   cdf['proc_param3']
    #   cdf['Rich_data_flag']
    if data.sid in [3, 5, 21]:
        data.RFI_rejection = cdf['RFI_rejection'][...]          # SID-  3,     5/21
    if data.sid in [3, 4, 20, 21]:
        data.complex     = cdf['complex'][...]              # SID-  3,4/20,  21,
    if data.sid in [3]:
        data.BG_subtract = cdf['BG_subtract'][...]          # SID-3    
        data.BG_select   = cdf['BG_select'][...]            # SID-3
        data.BG_downlink = cdf['BG_downlink'][...]          # SID-3
        data.Pol_sep_thres = cdf['Pol_sep_thres'][...]      # SID-3
        data.Pol_sep_SW  = cdf['Pol_sep_SW'][...]           # SID-3
    if data.sid in [4, 20]:
        data.N_block     = np.int64(cdf['N_block'][...])    # SID-4/20       
    if data.sid in [6, 22]:
        data.N_lag       = np.int64(cdf['N_lag'][...])          # SID-6/22
        #   cdf['sweep_table']
        #   cdf['FFT_window']
    if data.sid in [7, 23]:
        data.freq_center = cdf['freq_center'][...]              # SID-7/23
        data.N_block     = np.int16(cdf['N_block'][...])        # SID-7/23
        if data.sid in [7]:
            data.N_lag       = np.int16(cdf['N_lag'][...])      # SID-7
        if data.sid in [23]:
            data.N_feed      = np.int16(cdf['N_feed'][...])     # SID-23
        #    cdf['N_skip']

    # Header ##
    data.HF_QF       = cdf['HF_QF'][...]                        # all
    data.ADC_ovrflw  = cdf['ADC_ovrflw'][...]                   # SID-2,3,4/20,5/21,6/22,7/23
    #       cdf['pol']
    if data.sid in [2]:
        data.N_samp      = np.int64(cdf['N_samp'][...])         # SID-2
    if data.sid in [2, 3, 4, 20, 5, 21, 6, 22]:
        data.N_step      = np.int64(cdf['N_step'][...])         # SID-2,3,4/20,5/21,6/22
    if data.sid in [2, 3]:
        data.decimation  = cdf['decimation'][...]               # SID-2,3
    return data


def status_add(data, data1):
    # Common
    data.epoch       = np.r_["0", data.epoch, data1.epoch]
    data.scet        = np.r_["0", data.scet, data1.scet]

    # AUX
    data.T_RWI_CH1   = np.r_["0", data.T_RWI_CH1, data1.T_RWI_CH1]
    data.T_RWI_CH2   = np.r_["0", data.T_RWI_CH2, data1.T_RWI_CH2]
    data.T_HF_FPGA   = np.r_["0", data.T_HF_FPGA, data1.T_HF_FPGA]
    data.cal_signal  = np.r_["0", data.cal_signal, data1.cal_signal]
    #       cdf['Unique_ID']
    #       cdf['header_size']
    if data.sid in [2, 3, 4, 20, 5, 21, 6, 22, 7]:
        data.ch_selected = np.r_["0", data.ch_selected, data1.ch_selected]
    # if sid in [2, 3, 4, 20, 5, 21]:
    #   cdf['sweep_table']
    #   cdf['FFT_window']
    #   cdf['onboard_cal']
    #   cdf['proc_param0']
    #   cdf['proc_param1']
    #   cdf['proc_param2']
    #   cdf['proc_param3']
    #   cdf['Rich_data_flag']
    if data.sid in [3, 5, 21]:
        data.RFI_rejection = np.r_["0", data.RFI_rejection, data1.RFI_rejection]
    if data.sid in [3, 4, 20, 21]:
        data.complex     = np.r_["0", data.complex, data1.complex]
    if data.sid in [3]:
        data.BG_downlink = np.r_["0", data.BG_downlink, data1.BG_downlink]
        data.BG_subtract = np.r_["0", data.BG_subtract, data1.BG_subtract]
        data.BG_select   = np.r_["0", data.BG_select, data1.BG_select]
        data.Pol_sep_thres = np.r_["0", data.Pol_sep_thres, data1.Pol_sep_thres]
        data.Pol_sep_SW  = np.r_["0", data.Pol_sep_SW, data1.Pol_sep_SW]
    if data.sid in [4, 20]:
        data.N_block     = np.r_["0", data.N_block, data1.N_block]
    if data.sid in [6, 22]:
        data.N_lag       = np.r_["0", data.N_lag, data1.N_lag]
        #    cdf['sweep_table']
        #    cdf['FFT_window']
    if data.sid in [7, 23]:
        data.freq_center = np.r_["0", data.freq_center, data1.freq_center]
        data.N_block     = np.r_["0", data.N_block, data1.N_block]
        if sid in [7]:
            data.N_lag   = np.r_["0", data.N_lag, data1.N_lag]
        if sid in [23]:
            data.N_feed  = np.r_["0", data.N_feed, data1.N_feed]
        #    cdf['N_skip']

    # Header
    data.HF_QF       = np.r_["0", data.HF_QF, data1.HF_QF]
    data.ADC_ovrflw  = np.r_["0", data.ADC_ovrflw, data1.ADC_ovrflw]
    #       cdf['pol']
    if data.sid in [2]:
        data.N_samp      = np.r_["0", data.N_samp, data1.N_samp]
    if data.sid in [2, 3, 4, 20, 5, 21, 6, 22]:
        data.N_step      = np.r_["0", data.N_step, data1.N_step]
    if data.sid in [2, 3]:
        data.decimation  = np.r_["0", data.decimation, data1.decimation]
    return data


def status_shaping(data, index):
    # Common
    data.epoch       = data.epoch      [index]
    data.scet        = data.scet       [index]

    # AUX
    data.T_RWI_CH1   = data.T_RWI_CH1  [index]
    data.T_RWI_CH2   = data.T_RWI_CH2  [index]
    data.T_HF_FPGA   = data.T_HF_FPGA  [index]
    data.cal_signal  = data.cal_signal [index]
    #    cdf['Unique_ID']
    #    cdf['header_size']
    if data.sid in [2, 3, 4, 20, 5, 21, 6, 22, 7]:
        data.ch_selected = data.ch_selected[index]
    # if sid in [2, 3, 4, 20, 5, 21]:
    #   cdf['sweep_table']
    #   cdf['FFT_window']
    #   cdf['onboard_cal']
    #   cdf['proc_param0']
    #   cdf['proc_param1']
    #   cdf['proc_param2']
    #   cdf['proc_param3']
    #   cdf['Rich_data_flag']
    if data.sid in [3, 5, 21]:
        data.RFI_rejection = data.RFI_rejection[index]
    if data.sid in [3, 4, 20, 21]:
        data.complex     = data.complex    [index]
    if data.sid in [3]:
        data.BG_downlink = data.BG_downlink[index]
        data.BG_subtract = data.BG_subtract[index]
        data.BG_select   = data.BG_select  [index]
        data.Pol_sep_thres = data.Pol_sep_thres[index]
        data.Pol_sep_SW  = data.Pol_sep_SW [index]
    if data.sid in [4, 20]:
        data.N_block     = data.N_block    [index]
    if data.sid in [6, 22]:
        data.N_lag       = data.N_lag  [index]
        #    cdf['sweep_table']
        #    cdf['FFT_window']
    if data.sid in [7, 23]:
        data.freq_center = data.freq_center[index]
        data.N_block     = data.N_block    [index]
        if data.sid in [7]:
            data.N_lag   = data.N_lag      [index]
        if data.sid in [23]:
            data.N_feed  = data.N_feed     [index]

    # Header
    data.HF_QF       = data.HF_QF      [index]
    data.ADC_ovrflw  = data.ADC_ovrflw [index]
    #       cdf['pol']
    if data.sid in [2]:
        data.N_samp      = data.N_samp     [index]
    if data.sid in [2, 3, 4, 20, 5, 21, 6, 22]:
        data.N_step      = data.N_step     [index]
    if data.sid in [2, 3]:
        data.decimation  = data.decimation [index]

    return data


def status_nan(data, i):
    # AUX
    data.T_RWI_CH1    [i] = math.nan
    data.T_RWI_CH2    [i] = math.nan
    data.T_HF_FPGA    [i] = math.nan
    """
    data.cal_signal   [i] = math.nan
    #    cdf['Unique_ID']
    #    cdf['header_size']
    if sid in [2, 3, 4, 20, 5, 21, 6, 22, 7]:
        data.ch_selected  [i] = math.nan
    # if sid in [2, 3, 4, 20, 5, 21]:
    #   cdf['sweep_table']
    #   cdf['FFT_window']
    #   cdf['onboard_cal']
    #   cdf['proc_param0']
    #   cdf['proc_param1']
    #   cdf['proc_param2']
    #   cdf['proc_param3']
    #   cdf['Rich_data_flag']
    if sid in [3, 5, 21]:
        data.RFI_rejection[i] = math.nan
    if sid in [3, 4, 20, 21]:
        data.complex      [i] = math.nan
    if sid in [3]:
        data.BG_downlink  [i] = math.nan
        data.BG_subtract  [i] = math.nan
        data.BG_select    [i] = math.nan
        data.Pol_sep_thres[i] = math.nan
        data.Pol_sep_SW   [i] = math.nan
    if sid in [4, 20]:
        data.N_block      [i] = math.nan
    if sid in [6, 22]:
        data.RFI_rejection[i] = math.nan
        #    cdf['sweep_table']
        #    cdf['FFT_window']
        data.N_lag        [i] = math.nan
    if sid in [7, 23]:
        data.freq_center  [i] = math.nan
        data.N_block      [i] = math.nan
        if sid in [7]:
            data.N_feed   [i] = math.nan
        if sid in [23]:
            data.N_lag    [i] = math.nan
        #    cdf['N_skip']

    # Header
    data.ADC_ovrflw   [i] = math.nan
    #       cdf['pol']
    if sid in [2]:
        data.N_samp       [i] = math.nan
    if sid in [2, 3, 4, 20, 5, 21, 6, 22]:
        data.N_step       [i] = math.nan
    if sid in [2, 3]:
        data.HF_QF        [i] = math.nan
    """

    return data