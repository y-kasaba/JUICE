"""
    JUICE RPWI HF SID4 (Burst surv): L1a QL -- 2024/8/17
"""
import numpy as np
...
def hf_sid4_shaping(data, cal_mode, N_ch, comp_mode):
    """
    input:  data
    ...
    if cal_mode < 2 or N_ch < 4 or comp_mode < 4:
        ...
        data.EwEw        = data.EwEw[index[0]]
        # complex == 1:    # Matrix
        data.EuEv_re     = data.EuEv_re[index[0]]
        data.EvEw_re     = data.EvEw_re[index[0]]
        data.EwEu_re     = data.EwEu_re[index[0]]
        data.EuEv_im     = data.EuEv_im[index[0]]
        data.EvEw_im     = data.EvEw_im[index[0]]
        data.EwEu_im     = data.EwEu_im[index[0]]
        # complex == 3:    # 3D-matrix
        data.EuiEui      = data.EuiEui[index[0]]
        data.EuqEuq      = data.EuqEuq[index[0]]
        data.EviEvi      = data.EviEvi[index[0]]
        data.EvqEvq      = data.EvqEvq[index[0]]
        data.EwiEwi      = data.EwiEwi[index[0]]
        data.EwqEwq      = data.EwqEwq[index[0]]
        #
        data.EuiEvi      = data.EuiEvi[index[0]]
        data.EviEwi      = data.EviEwi[index[0]]
        data.EwiEui      = data.EwiEui[index[0]]
        data.EuqEvq      = data.EuqEvq[index[0]]
        data.EvqEwq      = data.EvqEwq[index[0]]
        data.EwqEuq      = data.EwqEuq[index[0]]
        #
        data.EuiEvq      = data.EuiEvq[index[0]]
        data.EuqEvi      = data.EuqEvi[index[0]]
        data.EviEwq      = data.EviEwq[index[0]]
        data.EvqEwi      = data.EvqEwi[index[0]]
        data.EwiEuq      = data.EwiEuq[index[0]]
        data.EwqEui      = data.EwqEui[index[0]]
        #
        data.EuiEuq      = data.EuiEuq[index[0]]
        data.EviEvq      = data.EviEvq[index[0]]
        data.EwiEwq      = data.EwiEwq[index[0]]
    ...
    for i in range(n_time):
        if data.complex[i] == 3:    # Matrix - N/R/L-separated
            ...
            data.EuEv_re[i] = data.EuiEvi[i] + data.EuqEvq[i];  data.EuEv_im[i] = -data.EuiEvq[i] + data.EuqEvi[i]
            data.EvEw_re[i] = data.EviEwi[i] + data.EvqEwq[i];  data.EvEw_im[i] = -data.EviEwq[i] + data.EviEvq[i]
            data.EwEu_re[i] = data.EwiEui[i] + data.EwqEuq[i];  data.EwEu_im[i] = -data.EwiEuq[i] + data.EwqEui[i]
