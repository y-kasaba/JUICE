"""
    JUICE RPWI HF SID7 (PSSR3 surv): L1a QL -- 2025/3/31
"""
import numpy as np
...
def hf_sid7_shaping(data):
    ...
    n_num = data.N_block[0] * data.N_samp[0]
    if n_num < data.auto_corr.shape[1]:
        print(" org:", data.auto_corr.shape, data.N_block[0], data.N_samp[0])
        data.auto_corr = data.auto_corr[:, 0:n_num]
        print(" cut:", data.auto_corr.shape, data.N_block[0], data.N_samp[0])
    data.auto_corr = np.array(data.auto_corr).reshape(data.n_time, data.N_block[0], data.N_samp[0])
